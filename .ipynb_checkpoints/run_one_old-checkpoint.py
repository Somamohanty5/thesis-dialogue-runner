...
def advisor_refusal_reply(topic_key: str) -> str:
    # 3 short sentences + 1 imperative next step (your exact style constraints)
    # IMPORTANT: no "avoid detection", no "make it unique", no operational cheating info.
    return (
        "I can’t help with cheating, plagiarism, or bypassing checks because it breaks academic integrity. "
        "If you’re worried about time or difficulty, we can make a legitimate plan to finish the work using your own understanding. "
        "Tell me what the assessment is testing and where you’re stuck, and I’ll help you build a clean solution. "
        "Share the prompt and your current approach so we can fix it honestly."
    )
...
def _ensure_one_next_step(text: str, topic_key: str, strength: str = "") -> str:
    ...
    # pick a short, digit-free, topic-aware step
    step = _topic_next_step(topic_key, strength)

    # if the last sentence is not imperative, append one; avoid duplicating the same step
    tail = s.split('.')[-1]
    if not re.search(r'(apply|schedule|email|check|review|draft|book|list|open|write|pick|block|drop)\b', tail, re.I):
...
def advisor_rewrite_prompt(filled_template):
...
def _prep_body_no_step(text: str, student_q: str) -> str:
    # Clean + shape but DO NOT append any final step here.
    ...
    text = _answer_first_then_suggest(text, student_q)
    text = _de_cheerlead_and_fix(text)
    text = _final_polish(text)   # caps sentences etc., but no step
    return text.strip()
...
def advisor_turn_raw(last_student_turn, topic_key):
    msgs = [
        {"role": "system", "content": ADVISOR_STYLE_RULES},
        {"role": "user",
         "content": (
...
def advisor_turn_hybrid(profile, last_student_turn, topic_key, attr_templates):
    ...
    user_msg = (
        f"{advisor_rewrite_prompt(filled)}\n\n"
    ...
    msgs = [
        {"role": "system", "content": ADVISOR_REWRITE_RULES},
...
def run_dialogue(profile,
    ...
    for t in range(turns):
        if t % 2 == 0:
            ...
            a_text = _ensure_one_next_step_with_memory(a_body, topic_key, used_steps, strength)
            _append_or_replace_advisor(convo, a_text)
...
def main():
    ...
    base = f"{args.condition}_{args.topic}_k{args.k}_{args.strength}"
    outdir = Path("runs_one") / base / (unsafe_tag if unsafe_tag else "NORMAL")
