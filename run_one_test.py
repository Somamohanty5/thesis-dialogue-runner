...
def _bad_student_question(q: str) -> bool:
    q = (q or "").strip()
    if not q:
        return True
    # only punctuation like "?" or "??"
...
def _fallback_followup_question(topic_key: str, last_advisor_line: str) -> str:
    # Simple, human follow-up that always makes sense for internships
    if (topic_key or "").lower() == "internships":
        return "How can I quickly tell if a project is strong enough to put on my resume?"
    # Generic fallback
    return "What should I do first based on what you just suggested?"
...
def _choose_step(topic_key: str, used_steps) -> str:
    ...
    if not pool:
        return "Pick one small step you can finish today"
    
    if used_steps:
        recent = {u.lower() for u in used_steps}
        pool2 = [x for x in pool if x.lower() not in recent]
...
- Do not start with “Great advice”, “Thanks”, “Got it”, or praise.
- Start directly with your question (or a short 3–6 word context, then the question).
...
def _strip_student_cheerleading(text: str) -> str:
    s = (text or "").strip()

    # Remove common gratitude/openers at the START of the message (and the rest of that clause)
    # Examples:
    # "Thanks! How do I...?"
    # "Thanks for the advice—how do I...?"
    # "Thank you so much. Can you...?"
    s = re.sub(
        r'^\s*(?:thanks|thank you)(?:\s+so\s+much)?'
        r'(?:\s+for\s+(?:the|your)\s+(?:help|advice|tips|suggestions))?'
        r'\s*(?:[,\-—:;!.\s]+)?',
        '',
        s,
        flags=re.I
    ...
    # Also remove "Great advice!" / "Got it!" type openers if you want
    s = re.sub(
        r'^\s*(?:got it|makes sense|great advice|good point|okay|ok)\s*(?:[,\-—:;!.\s]+)?',
        '',
        s,
        flags=re.I
    )

    # Clean broken leftover fragments like "for the advice!"
    s = re.sub(r'^\s*for\s+the\s+advice\s*[!.,\-—:;]*\s*', '', s, flags=re.I)

    # Fix spacing around punctuation
    s = re.sub(r'\s+([,.;!?])', r'\1', s)
    s = re.sub(r'([.!?]){2,}', r'\1', s)   # collapse !!! or .. into single
    s = re.sub(r'\s{2,}', ' ', s).strip()

    return s
def _strip_stray_bangs(text: str) -> str:
    s = (text or "").strip()

    # Remove leading stray "!" or "! " (common artifact)
    s = re.sub(r'^\s*!\s*', '', s)

    # Remove " . !" or " ! " after a period/space
    s = re.sub(r'\s+!\s+', ' ', s)
...
def student_turn(profile, topic_key, topic_text, ethics_block, styles, last_advisor_line=None):
    survey_style = pick_style(profile.get("student_id", 0), styles)

    # If advisor already spoke, force a natural follow-up reply that references advisor text
    if last_advisor_line:
        prompt = student_followup_prompt(topic_key, last_advisor_line, survey_style)
        msgs = [
        ...
        text = _strip_template_meta(text)
        text = _strip_unsupported_numbers(text)

        # Ensure it contains exactly ONE question (can be 1–2 sentences total)
        # Keep everything up to the first '?'
        m = re.search(r'^(.{0,400}?\?)', text)
        text = m.group(1).strip() if m else _force_single_question(text)

        # cap
        if len(text) > 220:
            text = text[:215].rsplit(" ", 1)[0] + "?"
        text = _strip_student_cheerleading(text)
        if _bad_student_question(text):
            text = _fallback_followup_question(topic_key, last_advisor_line)
        return text

    # Otherwise this is the FIRST student turn: generate a standalone question
    seed = _topic_seed_for_profile(topic_key, topic_text, profile)
    prompt = student_prompt(
        topic_key,
        seed,
        survey_style,
        ethics_block,
        last_advisor_line=None
    )
    ...
    text = _strip_template_meta(text)
    text = _strip_unsupported_numbers(text)
    text = _force_single_question(text)
    text = _de_advise_student_line(text)

    qs = re.findall(r'[^?]*\?', text)
    text = qs[0].strip() if qs else f"I'm stuck about {topic_key}; what's one concrete first step?"
...
def _prep_body_no_step(raw_text: str, last_student_turn: str) -> str:
    ...
    text = _kill_lists_and_headings(text)
    text = re.sub(r'^(thank you for your advice!?|thanks for your advice!?|great question!?)[\s,]*', '', text, flags=re.I)
    return text.strip()
    ...
    for t in range(turns):
        if t % 2 == 0:
            ...
            if is_unsafe_request(last_student):
                ...
                last_advisor_line = a_text.strip()
                continue
            ...
            a_body = _strip_stray_bangs(a_body) 
