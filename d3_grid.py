...
# IMPORTANT: Unsafe settings
K = 15
UNSAFE_PROB = 1.0          # guarantee one unsafe student turn per dialogue
UNSAFE_FIRST_TURN = False  # set True if you want unsafe always on first student turn

CONDITION = "D"            # use "B" or "D" for ethics conditions
TURNS_LIST = [6, 8, 12]
...
def run_one(cond, strength, topic, k, sid, turns, retries=2, sleep_between=1.0, ping_timeout=5.0):
    cmd = (
        ...
        f"--sid {sid} "
        f"--turns {turns} "
        f"--unsafe_prob {UNSAFE_PROB} "
    )
    if UNSAFE_FIRST_TURN:
        cmd += "--unsafe_first_turn "
    ...
    # Make logfile name include UNSAFE + prob + k
    logfile = logdir / f"UNSAFE_p{str(UNSAFE_PROB).replace('.','_')}_{cond}_{strength}_{topic}_k{k}_sid{sid}_t{turns}.log"
...
def main():
    # Safety: B or D only (since you said unsafe for B/D)
    if CONDITION not in ("B", "D"):
        raise SystemExit("Unsafe grid should use CONDITION='B' or 'D'.")
    ...
    for strength, turns in itertools.product(STRENGTHS, TURNS_LIST):
        runs.append({
            ...
            "unsafe_first_turn": UNSAFE_FIRST_TURN
        })

    Path("runs_grid").mkdir(exist_ok=True)
    Path("runs_grid/manifest_unsafe_9.json").write_text(json.dumps(runs, indent=2), encoding="utf-8")
